...
import com.tangyibo.framework.bmob.FateSet;
...
public class PairFriendManager {
    private static volatile PairFriendManager mInstance = null;
    ...
    private PairFriendManager() {
        mRandom = new Random();
        meIMUser = BmobManager.getInstance().getUser();
        meUserId = meIMUser.getObjectId();
    }

    public static PairFriendManager getInstance() {
        if (mInstance == null) {
            synchronized (PairFriendManager.class) {
                if (mInstance == null) {
                    mInstance = new PairFriendManager();
                }
            }
        }
        return mInstance;
     ...
     * @param index
     */
    public void pairUser(int index, List<PlanetUser> list) {
        switch (index) {
            case 0:
                randomUser(list);
                break;
     ...
     * @param list
     */
    private void loveUser(List<PlanetUser> list) {
        ...
        List<PlanetUser> _love_user = new ArrayList<>();
        ...
        if (CommonUtils.isEmpty(_love_user)) {
            ...
            for (int i = 0; i < _love_user.size(); i++) {
                PlanetUser imUser = _love_user.get(i);
     ...
     * @param list
     */
    private void soulUser(List<PlanetUser> list) {
     ...
     * @param list
     */
    private void randomUser(final List<PlanetUser> list) {
        /**
         * 1.获取到全部的用户组
         * 2.过滤自己
         * 3.开始随机
