...
import com.tangyibo.planet.model.AddFriendModel;
...
public class AddFriendActivity extends BaseBackActivity implements View.OnClickListener{
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.ll_to_contact:
                //处理权限
                LogUtils.i("跳转到通讯录");
                if (checkSinglePermission(Manifest.permission.READ_CONTACTS)) {
                    startActivity(new Intent(AddFriendActivity.this, ContactFriendActivity.class));
                } else {
                    requestMyPermission(new String[]{Manifest.permission.READ_CONTACTS});
                }
                break;
            case R.id.iv_search:
                LogUtils.i("搜索用户");
    ...
    private void queryPhoneUser() {
        ...
        //3.查询
        BmobManager.getInstance().queryPhoneUser(phone, new FindListener<PlanetUser>() {
            @Override
     ...
     * @param imUser
     */
    private void addContent(PlanetUser imUser) {
        AddFriendModel model = new AddFriendModel();
